...
async def upload_document(
    ...
    documents_service.document_upload(file, current_user.id, current_user.username, session)
    return
...
async def download_document(
    ...
    response = documents_service.download(idx, current_user.id, session)
    return response

@router.delete("/document", tags=['Document'], description="Route for deleting a document")
async def delete_document(
    ...
    documents_service.delete(idx, current_user.id, session, vector_session)
    return 